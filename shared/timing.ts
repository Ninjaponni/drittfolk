...
export function lineDuration(str: string): number {
  return Math.max(MIN_LINE_DURATION, Math.min(MAX_LINE_DURATION, str.length / CPS))
}

// Total server-timeout for en interaksjon
export function serverTimeout(insultText: string, replyText: string): number {
